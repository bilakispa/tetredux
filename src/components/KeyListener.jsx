...
// Actions
import { startGame, pauseGame, resumeGame, endGame, gameUpdateField } from '../actions/game';
import { drop, moveLeft, moveRight, getNextTetromino, rotate } from '../actions/tetromino';
...
export default function KeyListener () {
    ...
    const pausePressed = useKeyPress('p', () => setIsPressingPause(true), () => setIsPressingPause(false));
    const leftPressed = useKeyPress('ArrowLeft', () => setIsPressingLeft(true), () => setIsPressingLeft(false));
    const rightPressed = useKeyPress('ArrowRight', () => setIsPressingRight(true), () => setIsPressingRight(false));
    const downPressed = useKeyPress('ArrowDown', () => setIsPressingDown(true), () => setIsPressingDown(false));
    const rotatePressed = useKeyPress('ArrowUp', () => setIsPressingUp(true), () => setIsPressingUp(false));
    ...
    // Handlers
    const handleStartPressed = () => { // Start Game
        if (!gameStarted) {
            const startTetromino = getRandomTetromino();
            const nextTetromino = getRandomTetromino();

            dispatch(getNextTetromino({ startShape: startTetromino, newShape: nextTetromino }));
            dispatch(startGame());
        };
    }

    const handlePausePressed = () => { // Pause-Resume Game
        if (gameStarted) {
            if (!gamePaused) {
                dispatch(pauseGame());
            } else {
    ...
    const handleLeftPressed = () => { // Move Left
    ...
    const handleRightPressed = () => { // Move Right
        if (gameStarted && !gamePaused) {
            if (canMoveTo(field, currentShape[rotation], shapeX + 1, shapeY)) {
                dispatch(moveRight());
            }
    ...
    const handleDownPressed = () => { // Drop tetromino
        if (gameStarted && !gamePaused) {
            let endY = shapeY + 1;
            ...
            const { updatedField, score, gameOver } = mergeShapeWithField(field, currentShape[rotation], shapeX, endY - 1);
    ...
    const handleRotatePressed = () => { // Rotate Clockwise
        if (gameStarted && !gamePaused) {
